	...
	for (pkcs5_prf = FIRST_PRF_ID; pkcs5_prf <= WHIRLPOOL; pkcs5_prf++)
	{
			...
			cryptoInfo->mode++)
		{
				...
				cryptoInfo->ea = EAGetNext (cryptoInfo->ea))
			{
				...
				DecryptBlock(header + HEADER_ENCRYPTED_DATA_OFFSET, HEADER_ENCRYPTED_DATA_SIZE, HEADER_SIZE, 0, 4, cryptoInfo);

				// Magic 'TRUE'
				if (GetHeaderField32 (header, HEADER_OFFSET_MAGIC) != 0x54525545)
					continue;

                uint32_t crc = GetHeaderField32 (header, HEADER_OFFSET_CRC);
                if (crc != GetCrc32 (header + HEADER_OFFSET_DATA, HEADER_DATA_SIZE) )
                    continue;

                if ( GetHeaderField16(header, HEADER_OFFSET_DATASZ) > 0x100 )
                    continue;
                ...
                if (EAInit (cryptoInfo->ea, dk + primaryKeyOffset, cryptoInfo->ks) != 0 )
                {
                    status = ERR_MODE_INIT_FAILED;
                    goto err;
                }
...
int decode1(u8 *data, const char *pass, PCRYPTO_INFO *ci)
{
    ...
    return ReadHeader(0, data, &pwd, ci, NULL);
}
