...
uint8_t buffer[RNG_POOL_SIZE];
unsigned char *pRandPool = NULL;
static int bRandDidInit = 0;
int nRandIndex = 0, randPoolReadIndex = 0;
int HashFunction = DEFAULT_HASH_ALGORITHM;
int bDidSlowPoll = 0;	/* We do the slow poll only once */
int volatile bFastPollEnabled = 1;	/* Used to reduce CPU load when performing benchmarks */
int volatile bRandmixEnabled = 1;	/* Used to reduce CPU load when performing benchmarks */
...
#define RandaddInt32(x) RandAddInt((uint32_t)x);

void RandAddInt (uint32_t x)
{
	RandaddByte(x);
	RandaddByte((x >> 8));
	RandaddByte((x >> 16));
...
int volatile bThreadTerminate = 0;	/* This variable is shared among thread's so its made volatile */
...
int
RandgetBytes (unsigned char *buf, int len, int forceSlowPoll)
{
	int i;
	int ret = 1;
	...
	if (bDidSlowPoll == 0 || forceSlowPoll)
	{
		if (!SlowPoll ())
			ret = 0;
		else
	...
	{
		((uint32_t *) pRandPool)[i] = ~((uint32_t *) pRandPool)[i];
	}
...
MouseProc (int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	static uint32_t lastCrc, lastCrc2;
	...
	else
	{
		...
		if (crc != lastCrc && crc != lastCrc2)
		{
			...
			EnterCriticalSection (&critRandProt);
			RandaddInt32 ((uint32_t) (crc + timeCrc));
			LeaveCriticalSection (&critRandProt);
		}
		lastCrc2 = lastCrc;
		lastCrc = crc;
...
KeyboardProc (int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	if (nCapture)
	{
		...
		EnterCriticalSection (&critRandProt);
		RandaddInt32 ((uint32_t) (crc32int(&lParam) + timeCrc));
		LeaveCriticalSection (&critRandProt);
	}
